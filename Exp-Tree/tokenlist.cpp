...
TokenList::TokenList(const char str[])
{
  ...
  while(str[pos] != 0) // While we're not at the end of the end of the list
	...
	{ // Modified for string support
	  string s; 
	  if (isOp(str[pos]))
	    {
	      s.push_back(str[pos]); //  Check first character
	      while(isOp(str[pos+1]))
		{
		  pos++;
		  s.push_back(str[pos]);
		}
	    ...
	    {
	      s.push_back(str[pos]);
	      while((!isdigit(str[pos+1])) && (str[pos+1] != ' ') && (str[pos+1] != 0) && !isOp(str[pos+1]) && str[pos] !=0) // If the next element is a character that's not a space; making sure that we don't go past the end of the input string
		{
		  pos++;
		  s.push_back(str[pos]); // add it to the back of our string
		}
	      tok = new Token(s); // If str[pos] is not an integer, make an operator node
	    }
	}
      if(str[pos] != 0)
	{
